...
export default function Result() {
  const word = useAppSelector((state) => {
    return state.fetchedData?.value?.word;
  });
  const phonetics = useAppSelector((state) => {
    return state.fetchedData?.value?.phonetics;
  });
  const phonetic = useAppSelector((state) => {
    return state.fetchedData?.value?.phonetic;
  });
  const meanings = useAppSelector((state) => {
    return state.fetchedData?.value?.meanings;
  });
  const sourceUrls = useAppSelector((state) => {
    return state.fetchedData?.value?.sourceUrls;
  });
  const meaningResultOne = meanings?.[0].definitions.map(
    (element) => element.definition
  );
  const meaningResultTwoDefinition = meanings?.[1]
    ? meanings?.[1].definitions.map((element) => element.definition)
    : [];
  const meaningResultTwoExample = meanings?.[1]
    ? meanings?.[1].definitions
        .map((element) => element.example)
        .filter((example) => example !== undefined)
    : [];
  const audioFind = phonetics
    ?.map((element) => (element.audio !== '' ? element.audio : undefined))
    .filter(Boolean);
  ...
  return (
    <div>
      ...
      <ul className="w-full ml-5 list-disc">
        {meaningResultOne?.map((element: string, id: number) => (
          <li key={id} className="w-full py-2 pr-4">
            {element}
          </li>
        ))}
      </ul>
      {meanings?.[1] ? (
        <div className="pb-4 border-b border-gray-400">
          ...
          <div className="text-gray-400">Meaning(s)</div>
          <ul className="w-full ml-5 list-disc">
            {meaningResultTwoDefinition
              ? meaningResultTwoDefinition.map(
                  (element: string, id: number) => {
                    return (
                      <li key={id} className="w-full py-2 pr-5">
                        {element}
                        {meaningResultTwoExample.length !== 0 &&
                          meaningResultTwoExample.map(
                            (e: string, key: number) => (
                              <p
                                key={key}
